...
plot_correlation_merged_parental_mping <- function(x, y, title, ...){
    plot(x, y, pch=18, col='orange', xlab= '', ylab= '', main= title, cex.main=1, xlim=c(0, 300), ylim=c(0, 10))
    #regression line
    abline(lm(y~x), col="lightblue")
    reg <- lm(y~x)
    a <- round(reg$coefficients[[2]], 2)
    b <- round(reg$coefficients[[1]], 2)
    text(10, 8, pos=4, paste('Y = ', b, '+', paste(a, 'X', sep=''), sep=' '), cex=1)
    #corrlation
    cor <- cor.test(x, y)
    r2 <- round(cor$estimate[[1]], 2)
    p  <- cor$p.value
    ...
    p = signif(p, 2) 
    #}
    text(10, 9, pos=4, paste('R =', r2, ', P =', p, sep=' '), cex=1)
    xpos <- 150
    ypos <- 5
...
pdf('ping_number_clean_excision_cor_plot_AddRefmPing.pdf')

x   <- read.table('RIL272.RIL_mPing_Ping_Excision.Ref_mPing.table.txt', header=TRUE)
x_n <- read.table('RIL272.RIL_mPing_Ping_Excision.Ref_mPing.narrow_range.table.txt', header=TRUE)

print("Parental mPing vs Excision, RIL272")
cor.test(x[,2]+x[,11], x[,15])
print("Ping vs Excision, RIL272")
cor.test(x[,9], x[,15])

print("Parental mPing vs Excision, RIL272_narrow_range")
cor.test(x_n[,2]+x_n[,11], x_n[,15])
print("Ping vs Excision, RIL272_narrow_range")
cor.test(x_n[,9], x_n[,15])

par(mfrow=c(2,2))
plot_correlation_merged_parental_mping(x[,2]+x[,11], x[,15], '272 RILs')
plot_correlation_merged(x[,9], x[,15], '272 RILs')
plot_correlation_merged_parental_mping(x_n[,2]+x_n[,11], x_n[,15], '122 narrow range mPing RILs')
plot_correlation_merged(x_n[,9], x_n[,15], '123 narrow range mPing RILs')

par(mfrow=c(1,1))
plot_correlation_merged_parental_mping(x[,2]+x[,11], x[,15], 'Parental mPing Copy Number', 'Excision Number', '272 RILs')
plot_correlation_merged(x[,9], x[,15], 'Ping Copy Number', 'Excision Number', '272 RILs')
plot_correlation_merged_parental_mping(x_n[,2]+x_n[,11], x_n[,15], 'Parental mPing Copy Number', 'Excision Number', '122 narrow range mPing RILs')
plot_correlation_merged(x_n[,9], x_n[,15], 'Ping Copy Number', 'Excision Number', '122 narrow range mPing RILs')
