...
def readtable(infile, blackout):
    ofile = open(re.sub(r'\.list', '.blackout', infile), 'w')
    print >> ofile, re.sub(r'\.check\.list', r'', os.path.split(infile)[1])
    ...
    with open (infile, 'r') as filehd:
        for line in filehd:
            ...
            if len(line) > 2 and line.startswith(r'RIL'): 
                ...
                if not unit[18] == 'finished' and not unit[1] == 'NB' and not unit[9] == 'NB' and unit[-1] == 'Blackout':
                    #print line
                    ril = re.sub(r'RIL', r'', unit[0])
                    data[ril] = 1
                    if not blackout.has_key(ril):
