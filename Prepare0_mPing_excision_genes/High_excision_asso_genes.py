...
def main():
    ...
    mping_frequency  = read_mping_frq('RILs_ALL_fastq_correct_merged_duplicate_RelocaTEi.CombinedGFF.characterized.clean.shared_mping.ril.frequency')
    nearby_gene      = read_nearby_gene('mping.excision.non_ref.gff.mPing.annotation')
    gene_anno        = read_msu_anno('MSU7.gene.anno') 

    print 'mPing\tAllele Frequency\tDistance to Gene\tGene\tAnnotation\tIndependent Excisions' 
    for mping in sorted(high_excisions.keys()):
        print '%s\t%s\t%.2f\t%s\t%s\t%s\t%s' %(mping, mping_frequency[mping][0], float(mping_frequency[mping][1]), nearby_gene[mping][0], nearby_gene[mping][1], gene_anno[nearby_gene[mping][0]], high_excisions[mping])
