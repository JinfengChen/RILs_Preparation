...
#0	NA	RIL129
#1	C	RIL230
def summary_unique(ping_code, mping_table, unique, ping_number_sum, ping_single_sum, narrow):
    data = defaultdict(lambda : list())
    ril_mping =  read_mping_table(mping_table)
    ril_unique=  read_unique_table(unique, ril_mping, narrow)
...
def read_unique_table(infile, ril_mping, narrow):
    data  = defaultdict(lambda : list())
    r     = re.compile(r'RIL(\d+)')
    with open (infile, 'r') as filehd:
        for line in filehd:
...
def main():
    ...
    summary_unique(args.code, mping_table, unique, ping_number_sum, ping_single_sum, args.narrow)
