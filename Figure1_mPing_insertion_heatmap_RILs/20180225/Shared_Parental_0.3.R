pdf("Shared_Parental_0.3.pdf", width=8, height=14)
library(reshape)
library(ggplot2)
library(Rmisc)
#header <- c("NB", "HEG4", "A119", "A123", "IRIS_313.15904", "B160", "B235", "OtherStrains")
ind <- read.table("Shared_Parental_0.3.matrix", header=TRUE)
header <- colnames(ind)
ind_m <- as.matrix(ind)
#ind_m <- t(ind_m)
...
ggplot(data=ind_m_melt, aes(x=X2, y=X1)) + geom_tile(aes(fill=value), colour="snow2") + scale_fill_gradient(low='lightblue', high="steelblue") + theme_grey(base_size = base_size) + labs(x = "",y = "") + theme(legend.position = "none", axis.text = element_blank(), axis.ticks = element_blank()) + xlim(header)  + ylim(row.names(ind))
#heatmap(ind_m, Rowv=NA, Colv=NA, col=c('lightblue', 'steelblue'))
dev.off()
