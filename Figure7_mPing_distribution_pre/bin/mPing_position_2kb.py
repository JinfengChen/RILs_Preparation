...
def readtable(infile, mrnafile):
    ...
    for m in data.keys():
        ...
        if p == 'intergenic':
            if dist5.has_key(m):
                if dist5[m] >= -2000 and dist5[m] < 0:
                    p = '2k_upstream'
            if dist3.has_key(m):
                if dist3[m] >= -2000 and dist3[m] < 0:
                    p = '2k_downstream'
        ...
        sumx[p] += 1
        tt += 1
    types = ['intergenic','2k_upstream', 'five_prime_UTR', 'CDS', 'intron', 'three_prime_UTR','2k_downstream']
    prefix = os.path.splitext(infile)[0]
    ofile = open (prefix + '.position_2kb.distr', 'w')
    count = 0
    for p in types:
        count +=1
    ...
    ofile.close()
    #
    ofile = open (prefix + '.position_2kb.distr.list', 'w')
