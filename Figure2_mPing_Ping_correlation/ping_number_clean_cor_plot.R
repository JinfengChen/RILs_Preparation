plot_correlation_merged <- function(x, y, xlab, ylab, title, ...){
    plot(x, y, xlab= '', ylab= '', main= title, cex.main=1, xlim=c(0, 7), ylim=c(0, 140))
    #regression line
...
plot_correlation_merged_parental_mping <- function(x, y, title, ...){
    plot(x, y, pch=18, col='orange', xlab= '', ylab= '', main= title, cex.main=1, xlim=c(120, 260), ylim=c(0, 140))
    #regression line
    abline(lm(y~x), col="lightblue")
    reg <- lm(y~x)
    a <- round(reg$coefficients[[2]], 2)
    b <- round(reg$coefficients[[1]], 2)
    text(120, 100, pos=4, paste('Y = ', b, '+', paste(a, 'X', sep=''), sep=' '), cex=1)
    #corrlation
    cor <- cor.test(x, y)
    r2 <- round(cor$estimate[[1]], 2)
    p  <- cor$p.value
    ...
    text(120, 120, pos=4, paste('R2 =', r2, ', p-value <', p, sep=' '), cex=1)
    xpos <- 140
    ypos <- 44
    mtext("Parental", side=1,font=1, at=xpos+10,line=3, cex=0.8, col="black")
    mtext("mPing", side=1,font=3, at=xpos+38,line=3, cex=0.8, col="black")
    mtext("copy number", side=1,font=1, at=xpos+75,line=3, cex=0.8, col="black")
    mtext("Unique homozygous", side=2,font=1, at=ypos,line=3, cex=0.8, col="black")
...
pdf('ping_number_clean_cor_plot.pdf')
x <- read.table("RILs_ALL_fastq_correct_merged_duplicate_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.txt", header=T)
x_n <- read.table("RILs_ALL_fastq_correct_merged_duplicate_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.narrow_range.txt", header=T)
x_h <- read.table("RILs_ALL_fastq_correct_merged_duplicate_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.high_depth.txt", header=T)
x_hn <- read.table("RILs_ALL_fastq_correct_merged_duplicate_RelocaTEi.CombinedGFF.characterized.clean.mping.shared_unique_table.ping_code.high_narrow.txt", header=T)
hist(x[,2], xlab = 'HEG4 mPing copy number', main='')
plot(density(x[,2]), xlim=c(40, 300), xpd=TRUE, xlab = 'HEG4 mPing copy number', main='')
abline(v=c(173, 212), col=c('blue', 'blue'))
hom <- x[,6]
hom_n <- x_n[,6] 
hom_h <- x_h[,6]
hom_hn <- x_hn[,6]
...
plot_correlation_merged(ping_n, hom_n, 'Ping Copy Number', 'Unique homozygous mPing number', 'Parental mPing range (173-212)')
...
plot_correlation(ping_n, hom_n, 'Ping Copy Number', 'Unique homozygous mPing number', 'Parental mPing range (173-212)')
